...
| Language pickers | All modes | Target `Dropdown` bound to the 23 `{code, name}` constant verbatim (FR-5) and required everywhere; source in live/file is a read-only Auto-detect indicator (D2-A 2026-09-13: whisper runs `-l auto`, no source value is ever consumed). Any model×language combo is submittable; quality is measured, not gated. |
...
| Output cards | Text mode | Two cards (Formal / Informal) + origin-language caption (commentary cut per D1-A 2026-09-13). Each card has its own copy button. Cards render independently. |
...
| Device picker | Live mode | `Dropdown` listing available ffmpeg/avfoundation input devices queried each launch — no hard-coded names (FR-6). BlackHole shown only when present; its absence is never an error. File mode needs no device at all. |
