...
> Mirror copy. Canonical document lives at `_bmad-output/specs/spec-e1-shell-core/RETROSPECTIVE.md`; this copy satisfies the dispatch requirement that the retro report land in the implementation-artifacts area. No `sprint-status.yaml` write was performed (stories mode).
...
- Report placement: canonical document at `{spec-folder}/RETROSPECTIVE.md` per the skill's stories-mode rule, plus this verbatim mirror at `_bmad-output/implementation-artifacts/epic-1-retro-2026-09-09.md` per the invocation's implementation-artifacts requirement. No `sprint-status.yaml` write, no edits to `SPEC.md`, `stories.yaml`, or story artifacts.
