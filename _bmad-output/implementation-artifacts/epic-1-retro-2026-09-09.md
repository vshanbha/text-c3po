...
## Post-acceptance hardening (2026-09-09, commit 6abf0aa)

Operator-tested follow-up after the accepted verdict; behavior verified live
plus gate re-run (lfm2.5 25/25 PASS, recorded in research.md).

- Streaming JSON transport: same single prompt and schema, tokens accumulate
  and parse once; live Formal preview plus throttled progress; Stop closes the
  stream (server aborts mid-generation, verified against Ollama disconnect
  behavior) with generation-guarded stale drops.
- Speed learnings: (1) thinking phase ~10s/call, disabled via reasoning=False
  (first token 5.5s to 0.7s); (2) num_ctx=4096 pin removed — ollama ps shows
  128K on GPU and the pin forced per-call model reloads; (3) paragraph
  splitter removed — it multiplied first-token wait N times, single call is
  clean; (4) streaming overhead ~0.6s accepted for progress plus true Stop.
- Packaging: installable src-layout package, no PYTHONPATH (`uv pip install
